...
let iconvLite = null;
try {
  // Optional dependency fallback for runtimes where TextDecoder('windows-1251') is unavailable.
  ({ default: iconvLite } = await import('iconv-lite'));
} catch {
  iconvLite = null;
}
...
function decodeXmlBuffer(buf) {
  ...
  try {
    ...
    if (iconvLite) {
      return iconvLite.decode(buf, 'win1251');
    }
    return utf8;
  }
}
